...
public class JwtSigningOptions
{
    ...
    ///     When an HMAC algorithm is chosen, the SigningKey setting will be used as both the signing key and the verifying
    ///     key.
    ///     To use asymmetric RSA signing and verification, the following algorithms may be used: 'RS256', 'RS384', 'RS512'.
    ///     When an RSA algorithm is chosen, the SigningKey setting must be set to an RsaSecurityKey that contains an RSA
    ///     private key.
