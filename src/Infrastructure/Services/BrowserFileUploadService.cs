...
/// </summary>
public class BrowserFileUploadService : IFileUploadService
{
    ...
    /// <returns>The result of the file upload operation.</returns>
    public async Task<Result<UploadedFileInfo>> UploadAsync(UploadRequest request)
    {
        try
        {
            ...
            {
                return Result<UploadedFileInfo>.Failure("No file data provided");
            }

            var folder = request.UploadType.GetDisplayName();
        ...
        {
            return Result<UploadedFileInfo>.Failure($"Failed to upload file: {ex.Message}");
        }
    }
    /// <summary>
    /// remove file
    ...
    {
        var removefile = Path.Combine(Directory.GetCurrentDirectory(), uri);
